...
var calcChecksum = function (entropy) {
    // Checksum is ent.length/4 bits long
    var bitsLeft = 8 - entropy.length / 4;
    // Zero rightmost "bitsLeft" bits in byte
    // For example: bitsLeft=4 val=10111101 -> 10110000
    return new Uint8Array([((0, sha256)(entropy)[0] >> bitsLeft) << bitsLeft]);
};
function getCoder(wordlist) {
    ...
    wordlist.forEach(function (i) {
        if (typeof i !== 'string')
            throw new Error("Wordlist: non-string element: ".concat(i));
    });
    return base.utils.chain(base.utils.checksum(1, calcChecksum), base.utils.radix2(11, true), base.utils.alphabet(wordlist));
}
/**
...
function mnemonicToEntropy(mnemonic, wordlist) {
    var words = normalize(mnemonic).words;
...
function mnemonicToSeed(mnemonic, passphrase) {
    ...
    return (0, pbkdf2_1.pbkdf2Async)(sha512, normalize(mnemonic).nfkd, salt(passphrase), { c: 2048, dkLen: 64 });
}
...
function mnemonicToSeedSync(mnemonic, passphrase) {
    if (passphrase === void 0) { passphrase = ''; }
    return (0, hashmini.pbkdf2)(hashmini.sha512, normalize(mnemonic).nfkd, salt(passphrase), { c: 2048, dkLen: 64 });
}
