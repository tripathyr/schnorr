...
//ROHIT adding Taproot Input - Output integration
var bech32mInputs = [];
var bech32mOutputs = [];
var tapBech32mIns = [];
var tapBech32mOuts = [];

function addBech32mInput(txidHex, hash, index, amount, scriptHex) {
  bech32mInputs.push({
    txid: base.hex.decode(txidHex),
    hash: hash,
    index: index,
    amount: BigInt(amount),
    script: base.hex.decode(scriptHex)
  });
}
function addBech32mOutput(address, amount) {
  bech32mOutputs.push([address, BigInt(amount)]);
}
...
function _redeemScript(addr, key) {
    ...
    function createTransaction(senders, redeemScripts, receivers, amounts, fee, change_address, fee_from_receiver) {
        return new Promise((resolve, reject) => {
            ...
            addInputs(tx, senders, redeemScripts, total_amount, fee, output_size, fee_from_receiver).then(result => {
                ...
                tapBech32mIns = tx.ins;
                tapBech32mOuts = tx.outs;
                resolve(result);
            }).catch(error => reject(error))
        })
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            fetch_api(`unspent?active=${addr}`).then(result => {
                ...
                for (let i = 0; i < utxos.length && required_amount > 0; i++) {
                    ...
                    tx.addinput(utxos[i].tx_hash_big_endian, utxos[i].tx_output_n, utxos[i].script, 0xfffffffd /*sequence*/); //0xfffffffd for Replace-by-fee
                    //ROHIT Add the input to hand over data to bech32m side
                    addBech32mInput(utxos[i].tx_hash, utxos[i].tx_hash_big_endian, utxos[i].tx_output_n, utxos[i].value, utxos[i].script);
    ...
    function addOutputs(tx, receivers, amounts, change_address) {
        ...
        for (let i in receivers) {
            ...
            //ROHIT Taproot Output addition
            addBech32mOutput(receivers[i], amounts[i]);
        ...
        //ROHIT Taproot Output addition
        addBech32mOutput(change_address, 0);
    ...
    function edit-Fee(tx, current_fee, target_fee, index = -1) {
